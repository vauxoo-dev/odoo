...
# TODELETE: when set, every "id" sequence starts past the largest 32-bit
# integer, so the whole instance is built with identifiers that do not fit in
# an int4. Anything still handling ids as 32-bit fails loudly instead of
# waiting for the database to grow into the problem.
BIGINT_STRESS = any(map(os.getenv, ('ODOO_BIGINT_STRESS',)))
INT4_MAX = 2**31 - 1
