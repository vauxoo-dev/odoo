...
class TestTrackingValueWidth(MailCommon):
    ...
    def test_tracked_many2one_stores_its_id(self):
        """ tracking a many2one stores its id in the integer column """
        model = self.env['ir.model'].create({
            'name': 'Tracked Model', 'model': 'x_tracked_m2o', 'is_mail_thread': True,
        })
        self.env['ir.model.fields'].create({
            'name': 'x_partner_id', 'model_id': model.id, 'ttype': 'many2one',
            'relation': 'res.partner', 'tracking': True,
        })
        record = self.env['x_tracked_m2o'].create({})
        partner = self.env['res.partner'].create({'name': 'Tracked Partner'})
        record.write({'x_partner_id': partner.id})
        record.env.flush_all()

        tracking = self.env['mail.tracking.value'].search(
            [('new_value_integer', '=', partner.id)], limit=1)
        self.assertTrue(tracking, "the id of the tracked many2one was not stored")
        self.assertEqual(tracking.new_value_integer, partner.id)
