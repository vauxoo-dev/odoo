...
class Pricelist(models.Model):
    ...
    def _compute_price_rule(self, products_qty_partner, date=False, uom_id=False):
        ...
        for product, qty, partner in products_qty_partner:
            ...
            for rule in items:
                if not rule._check_pricelist_rule(product, partner, qty_in_product_uom):
                    continue
...
class PricelistItem(models.Model):
    ...
    def _check_pricelist_rule(self, product, partner, qty_in_product_uom):
        """Method to check if the item is a suitable one for the quantity that is going to be need it, a certain
        product and partner, if all the conditions are pass we are returning True. If the rule isn't suitable we are
